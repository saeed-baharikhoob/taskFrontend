...
import { PriceChange } from "@/components/features/token/token-overview";
...
function ShareToken({
  ...
  const sharePage = async () => {
    window.setTimeout(() => {
      const e = document.getElementById("screenshot-token");
      if (e && values) {
        htmlToImage
          .toPng(e)
  ...
  return (
    <>
      <Dialog>
        ...
        <DialogContent>
          <DialogHeader>
            <DialogTitle className="font-bold">Share Token Details</DialogTitle>
          </DialogHeader>
          <div id="screenshot-token" className="bg-secondary p-4 rounded-lg">
            <div className="flex items-center gap-3 justify-center">
              <Avatar className="w-20 h-20 md:h-10 md:w-10">
                <AvatarImage
                  src={logo ?? ""}
              ...
              <h1 className="font-bold md:text-lg xl:text-2xl m-0 p-0 mr-2">
                {minifyTokenName(
                  token?.data?.[0]?.attributes?.name?.split("/")[0] || "N/A"
                )}
              </h1>
              <h2 className="text-sm m-0 p-0 font-light">
                {minifyTokenName(token?.data?.[0]?.attributes?.name || "N/A")}
              </h2>
            </div>
            <div className="flex items-stretch justify-between gap-3 mt-4">
              <div className="w-[calc(33%-32px)]">
                ...
                <div className="flex flex-col items-center gap-1 my-3">
                  <div className="text-xs">Holder interest</div>
                  <div className="text-xl">
              ...
              <div className="w-[calc(33%-32px)]">
                <div className="flex flex-col items-center gap-1 my-3">
                  ...
                  <div className="text-xl">
                    ...
                    {formatCash(
                      +parseFloat(
                        token?.data?.[0]?.attributes?.volume_usd?.["h24"] || "0"
                      ).toFixed(2)
                    )}
                  </div>
                </div>
                <div className="flex flex-col items-center gap-1 my-3">
                  ...
                  <div className="text-xl">
                    {dayjs(
                      token?.data?.[0]?.attributes?.pool_created_at
                    ).fromNow()}
                  </div>
                </div>
              </div>
              <div className="w-[calc(33%-32px)] flex flex-col items-center justify-center">
                ...
                <div className="border border-border flex items-center justify-center mt-3 p-2 rounded-2xl w-full text-xs gap-2">
                  <Image
                    src={`/networks/${token?.data?.[0]?.id?.split("_")[0]}.png`}
                    width={14}
                    height={14}
                    alt=""
                  />
                  <div className="mb-1">{token?.data?.[0]?.id?.split("_")[0] || "N/A"}</div>
                </div>
              </div>
            </div>
          </div>
